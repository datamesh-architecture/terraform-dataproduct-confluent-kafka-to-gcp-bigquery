...
resource "confluent_connector" "sink" {
  ...
  config_sensitive = {
    "keyfile" = base64decode(google_service_account_key.kafka_sink_gcp_service_account_key.private_key)
  }
  ...
  config_nonsensitive = {
    ...
    "name"                     = "BigQueryConnector_${join("_", local.kafka_topics)}"
    "kafka.auth.mode"          = "SERVICE_ACCOUNT"
    "kafka.service.account.id" = confluent_service_account.app-connector.id
    "input.data.format"        = "JSON"
    "tasks.max"                = "1"

    # this will not override existing partitioning configuration
  ...
  depends_on = [
    google_bigquery_dataset_iam_binding.kafka_sink_dataset_iam_binding,
